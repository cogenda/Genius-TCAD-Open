...
public:
  ...
  PetscScalar ElecGenRate (const PetscScalar &Tl,const PetscScalar &Ep,const PetscScalar &Eg) const
  {
    ...
    PetscScalar threshold = (Ep+c)/d;
    PetscScalar smooth_rate;
    if (threshold < 0.1)
      smooth_rate = 0;
    else if (threshold > 0.2)
      smooth_rate = 1;
    else
      smooth_rate = 10*(threshold-0.1);

    return smooth_rate*Ep/(a+b*exp(d/(Ep+c)));
  }
  ...
  AutoDScalar ElecGenRate (const AutoDScalar &Tl,const AutoDScalar &Ep,const AutoDScalar &Eg) const
  {
    ...
    AutoDScalar threshold = (Ep+c)/d;
    AutoDScalar smooth_rate;
    if (threshold < 0.1)
      smooth_rate = 0;
    else if (threshold > 0.2)
      smooth_rate = 1;
