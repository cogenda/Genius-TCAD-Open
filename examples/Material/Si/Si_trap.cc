...
public:
  ...
  int calibrate( std::vector<Parser::Parameter> & pmi_parameter)
  {
    ...
    if(type == Interface && !has_interface_name)
    {
      _calibrate_error_info += "Trap: no interface name specified for interface trap";
      return 1;
    }

    if(type == Interface && !has_interface_density)
    {
      _calibrate_error_info += "Trap: no interface charge density specified for interface trap";
      return 1;
    }
