...
class GSS_Si_Mob_Sdevice : public PMIS_Mobility
{
private:
  ...
  PetscScalar VSATN0;
  PetscScalar VSATP0;
  PetscScalar VSATN_EXP;
  PetscScalar VSATP_EXP;
  ...
  void Mob_Lucent_Init()
  {
    ...
    VSATN0    =  1.0700000E7*cm/s;
    VSATN_EXP =  0.87;
    VSATP0    =  3.8700000E6*cm/s;
    VSATP_EXP =  0.52;
         ...
         parameter_map.insert(para_item("VSATN.EXP", PARA("VSATN.EXP",    "", "-", 1.0, &VSATN_EXP)) );
         parameter_map.insert(para_item("VSATP.EXP", PARA("VSATP.EXP",    "", "-",1.0 , &VSATP_EXP)) );
#endif
...
public:
  ...
  {
    PetscScalar vsat = VSATN0*std::pow(T300/Tl, VSATN_EXP);
  ...
  {
    AutoDScalar vsat = VSATN0*adtl::pow(T300/Tl, VSATN_EXP);
  ...
  {
    PetscScalar vsat = VSATP0*std::pow(T300/Tl, VSATP_EXP);
  ...
  {
    AutoDScalar vsat = VSATP0*adtl::pow(T300/Tl, VSATP_EXP);
...
// constructor
public:
  GSS_Si_Mob_Sdevice(const PMIS_Environment &env):PMIS_Mobility(env)
  {
    PMI_Info = "This is the Sdevice compatable Lucent mobility model of Silicon";
    Mob_Lucent_Init();
  }
...
extern "C"
{
  DLL_EXPORT_DECLARE PMIS_Mobility* PMIS_Si_Mob_Sdevice (const PMIS_Environment& env)
  {
    return new GSS_Si_Mob_Sdevice(env);
  }
}
