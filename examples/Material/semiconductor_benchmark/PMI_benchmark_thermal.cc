  ...
  :dll_file(0)
{
  ...
  // open thermal model
  std::string model_fun_name = "PMIS_" + material + "_Thermal_Default";
...
void PMI_Benchmark_Thermal::set_doping(double Na, double Nd)
{
  thermal->SetFakeDopingEnvironment(Na*pow(cm,-3), Nd*pow(cm,-3));
}


void PMI_Benchmark_Thermal::set_mole(double mole_x, double mole_y)
{
  thermal->SetFakeMoleEnvironment(mole_x, mole_y);
}
