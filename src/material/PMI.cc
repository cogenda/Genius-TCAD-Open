...
{
  std::map<const std::string, PARA>::iterator it = parameter_map.find(var_name);
 ...
 */
std::map<const std::string, PARA > & PMI_Server::get_parameter_info()
{
...
std::string PMI_Server::get_parameter_string(const int verbosity)
{
  ...
  for( std::map<const std::string, PARA>::const_iterator it = parameter_map.begin();
