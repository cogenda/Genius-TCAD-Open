...
class GSS_AlGaAs_Trap_Default : public PMIS_Trap
{
private:
  // {{{ class TrapSpec
  class TrapSpec
...
public:
  ...
  GSS_AlGaAs_Trap_Default(const PMIS_Environment &env):PMIS_Trap(env)
  {
    PMI_Info = "This is the Default carrier trapping model of AlGaAs";
    Trap_Init();
  }
...
extern "C"
{
  DLL_EXPORT_DECLARE  PMIS_Trap* PMIS_AlGaAs_Trap_Default (const PMIS_Environment& env)
  {
    return new GSS_AlGaAs_Trap_Default(env);
  }
}
