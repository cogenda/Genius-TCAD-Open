...
private:
  ...
  class TrapSpec
  {
    public:
      ...
      /******** for interface traps stored as profile ************************/
      std::string interface_profile_name;
    ...
    public:
      ...
      TrapSpec(const std::string &profile, const PetscScalar &pf, const TrapChargeType ct, const TrapType t, const PetscScalar &Et, const PetscScalar &sn, const PetscScalar &sp, const PetscScalar &gn, const PetscScalar &gp)
      {
        if (t==Bulk)
        {
          // bulk trap
        ...
        {
          // interface trap
          interface_profile_name = profile;
          prefactor = pf;
        }
        charge_type    = ct;
        type = t;
      ...
      // interface trap by interface name and density
      TrapSpec(const std::string &interface, const PetscScalar &dit, const PetscScalar &pf, const TrapChargeType ct, const PetscScalar &Et, const PetscScalar &sn, const PetscScalar &sp, const PetscScalar &gn, const PetscScalar &gp)
      {
        // interface trap
...
public:
  ...
  {
    TrapSpec s(profile_name, prefactor, ct, Bulk, Et,sn,sp,gn,gp);
   ...
   */
  int AddInterfaceTrapSpec(const std::string &profile_name, const PetscScalar &prefactor, const TrapChargeType ct, const PetscScalar &Et, const PetscScalar &sn, const PetscScalar &sp, const PetscScalar &gn, const PetscScalar &gp)
  {
    TrapSpec s(profile_name, prefactor, ct, Interface, Et,sn,sp,gn,gp);
    TrapSpecs.push_back(s);
    return (TrapSpecs.size()-1);
  }
   ...
   * But the traps are not actually created until the PMIs are initilized with init_bc_node().
   */
  int AddInterfaceTrapSpec(const std::string &if_name, const PetscScalar &interface_density, const PetscScalar &prefactor, const TrapChargeType ct, const PetscScalar &Et, const PetscScalar &sn, const PetscScalar &sp, const PetscScalar &gn, const PetscScalar &gp)
  {
    TrapSpec s(if_name, interface_density, prefactor, ct, Et,sn,sp,gn,gp);
    TrapSpecs.push_back(s);
    return (TrapSpecs.size()-1);
  }
  ...
  void init_bc_node(const std::string & bc_label)
  {
    for (unsigned int i=0; i<TrapSpecs.size(); i++)
    {
      ...
      PetscScalar conc = 0.0;
      
      if(!TrapSpecs[i].interface_profile_name.empty())
        conc += ReadRealVariable(TrapSpecs[i].interface_profile_name)*TrapSpecs[i].prefactor;
      
      if (TrapSpecs[i].interface_name == bc_label)
        conc += TrapSpecs[i].interface_density*TrapSpecs[i].prefactor;
  ...
  int calibrate( std::vector<Parser::Parameter> & pmi_parameter)
  {
    ...
    if(type == Bulk && !has_profile_name)
    {
      _calibrate_error_info += "Trap: no profile name specified for bulk trap";
      return 1;
    }

    if(!profile_name.empty() && !HasVariable(profile_name))
    {
      _calibrate_error_info += "Trap: profile " + profile_name + " not exist";
      return 1;
    }

    // for interface trap, we must have interface name
    // for bulk trap, we must have profile name
    if(type == Interface && (!has_interface_name && !has_profile_name) )
    {
      _calibrate_error_info += "Trap: no interface trap specified for interface trap";
      return 1;
    }

    if(!has_charge_type)
    {
      _calibrate_error_info += "Trap: no charge type for the trap specified";
      return 1;
    }
    ...
    if (type==Interface)
    {
      ...
      if(has_profile_name)
      {
        AddInterfaceTrapSpec(profile_name,prefactor,charge_type,Et,sigman,sigmap,gn,gp);
      }
      
      if(has_interface_name)
      {
        AddInterfaceTrapSpec(interface_name,Dit,prefactor,charge_type,Et,sigman,sigmap,gn,gp);
      }
    }
    else
    {
  ...
  const std::string & get_parameter_string(const int verbosity)
  {
    ...
    for ( std::vector<TrapSpec>::const_iterator it = TrapSpecs.begin();
          it != TrapSpecs.end(); it++ )
    {
      if (it->type != Interface || it->interface_name.empty())
        continue;
    ...
    for ( std::vector<TrapSpec>::const_iterator it = TrapSpecs.begin();
          it != TrapSpecs.end(); it++ )
    {
      if (it->type != Interface || it->interface_profile_name.empty())
        continue;
      ...
      {
        output << "Interface traps:" << std::endl;
        output << std::setw(wd_name)   << "Interface Profile"
               << std::setw(wd_narrow) << "Et (eV)"
               << std::setw(wd_wide)   << "sigma_n (cm^-2)"
               << std::setw(wd_wide)   << "sigma_p (cm^-2)"
               << std::setw(wd_narrow) << "g_n"
      ...
      output   << std::setw(wd_name)   << it->interface_profile_name
               << std::setw(wd_narrow) << it->E_t
               << std::setw(wd_wide)   << it->sigma_n / (cm*cm)
               << std::setw(wd_wide)   << it->sigma_p / (cm*cm)
               << std::setw(wd_narrow) << it->g_n
