...
class GSS_NPolySi_Thermal : public PMIC_Thermal
{
private:
  ...
  void   Thermal_Init()
  {
    ...
    B_SP_HEA =  4.420000e-01*J/kg/std::pow(K,2);
...
public:
  ...
  GSS_NPolySi_Thermal(const PMIC_Environment &env):PMIC_Thermal(env)
  {
    Thermal_Init();
  }
  ~GSS_NPolySi_Thermal()
...
extern "C"
{
  DLL_EXPORT_DECLARE  PMIC_Thermal* PMIC_NPolySi_Thermal_Default (const PMIC_Environment& env)
  {
    return new GSS_NPolySi_Thermal(env);
  }
}
