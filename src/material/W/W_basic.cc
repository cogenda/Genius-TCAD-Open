...
class GSS_W_BasicParameter : public PMIC_BasicParameter
{
private:
  ...
  void   Basic_Init()
  {
    ...
    PERMEABI = 1.0;
    AFFINITY = 4.5*eV;
    DENSITY  = 19.25000e-03*kg*std::pow(cm,-3);
    IONDENSITY  = 1.000000e+23*std::pow(cm,-3);
    CONDUCTANCE = 1.8e7*A/m/V;
...
public:
  ...
  {
    atoms.push_back(Atom("Wolfram",   "W",  74, 183.85));//Wolfram
    fraction.push_back(1.0);
  }

public:
  GSS_W_BasicParameter(const PMIC_Environment &env):PMIC_BasicParameter(env)
  {
    Basic_Init();
  }
  ~GSS_W_BasicParameter()
  {
  }
}
;
...
extern "C"
{
  DLL_EXPORT_DECLARE  PMIC_BasicParameter* PMIC_W_BasicParameter_Default (const PMIC_Environment& env)
  {
    return new GSS_W_BasicParameter(env);
  }
}
