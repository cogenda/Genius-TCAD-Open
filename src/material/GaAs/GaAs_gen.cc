...
private:
  ...
  void 	Avalanche_Init()
  {
    ...
    T300      = 300.0*K;
#ifdef __CALIBRATE__
    parameter_map.insert(para_item("N.IONIZA",  PARA("N.IONIZA",    "Constant term in the prefactor of the electron ionization coefficient.", "cm^-1", 1.0/cm, &N_IONIZA)) );
    parameter_map.insert(para_item("EXN.II",    PARA("EXN.II",      "Exponent of the E-field dependence in the exponential factor of the electron ionization coefficient.", "-", 1.0, &EXN_II)) );
    parameter_map.insert(para_item("N.ION.1",   PARA("N.ION.1",     "First-order temperature coefficient in the prefactor factor of the electron ionization coefficient.", "cm^-1 K^-1", 1.0/cm/K, &N_ION_1)) );
    parameter_map.insert(para_item("N.ION.2",   PARA("N.ION.2",     "Second-order temperature coefficient in the prefactor factor of the electron ionization coefficient.", "cm^-1 K^-2", 1.0/cm/K/K, &N_ION_2)) );
    parameter_map.insert(para_item("LAN300",    PARA("LAN300",      "Optical phonon mean free path for electrons.", "cm", cm, &LAN300)) );

    parameter_map.insert(para_item("P.IONIZA",  PARA("N.IONIZA",    "Constant term in the prefactor of the hole ionization coefficient.", "cm^-1", 1.0/cm, &P_IONIZA)) );
    parameter_map.insert(para_item("EXP.II",    PARA("EXP.II",      "Exponent of the E-field dependence in the exponential factor of the hole ionization coefficient.", "-", 1.0, &EXP_II)) );
    parameter_map.insert(para_item("P.ION.1",   PARA("P.ION.1",     "First-order temperature coefficient in the prefactor factor of the hole ionization coefficient.", "cm^-1 K^-1", 1.0/cm/K, &P_ION_1)) );
    parameter_map.insert(para_item("P.ION.2",   PARA("P.ION.2",     "Second-order temperature coefficient in the prefactor factor of the hole ionization coefficient.", "cm^-1 K^-2", 1.0/cm/K/K, &P_ION_2)) );
    parameter_map.insert(para_item("LAP300",    PARA("LAP300",      "Optical phonon mean free path for holes.", "cm", cm, &LAP300)) );

    parameter_map.insert(para_item("OP.PH.EN",  PARA("OP.PH.EN",    "Mean optical phonon energy used in the temperature dependence of carrier energy mean free path.", "eV", eV, &OP_PH_EN)) );

    parameter_map.insert(para_item("ELEC.TAUW", PARA("ELEC.TAUW",   "Time constant of electron energy relaxation via impact ionization.", "s", s, &ElecTauw)) );
    parameter_map.insert(para_item("HOLE.TAUW", PARA("HOLE.TAUW",   "Time constant of hole energy relaxation via impact ionization.", "s", s, &HoleTauw)) );
#endif
  }
public:
  //---------------------------------------------------------------------------
...
extern "C"
{
  DLL_EXPORT_DECLARE  PMIS_Avalanche* PMIS_GaAs_Avalanche_Selberherr (const PMIS_Environment& env)
  {
    return new GSS_GaAs_Avalanche(env);
  }

  DLL_EXPORT_DECLARE  PMIS_Avalanche* PMIS_GaAs_Avalanche_Default (const PMIS_Environment& env)
  {
    return new GSS_GaAs_Avalanche(env);
  }
}
