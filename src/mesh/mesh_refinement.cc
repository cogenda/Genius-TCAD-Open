// $Id: mesh_refinement.cc,v 1.2 2008/03/20 13:33:40 gdiso Exp $
...
bool MeshRefinement::refine_and_coarsen_elements (const bool maintain_level_one)
{
  ...
  if (_maintain_level_one)
    genius_assert(test_level_one(true));
  ...
  do
  {
    const bool coarsening_satisfied =  this->make_coarsening_compatible(maintain_level_one);

    const bool refinement_satisfied =  this->make_refinement_compatible(maintain_level_one);

    bool smoothing_satisfied =  !this->eliminate_unrefined_patches();

    bool interface_satisfied = this->make_interface_compatible();
  ...
  if (_maintain_level_one)
    genius_assert(test_level_one(true));
  genius_assert(this->make_coarsening_compatible(maintain_level_one));
  genius_assert(this->make_refinement_compatible(maintain_level_one));
  ...
  // genius_assert(!this->eliminate_unrefined_patches());
  ...
  if (_maintain_level_one)
    genius_assert(test_level_one(true));
  genius_assert(test_unflagged(true));
  genius_assert(this->make_coarsening_compatible(maintain_level_one));
  genius_assert(this->make_refinement_compatible(maintain_level_one));
  // FIXME: This won't pass unless we add a redundant find_neighbors()
  // call or replace find_neighbors() with on-the-fly neighbor updating
          ...
          for (unsigned int n=0; n<elem->n_neighbors(); n++)
            if (elem->neighbor(n) != NULL)     // I have a neighbor
              ...
              {  // We need to find which of its children
                ...
                genius_assert(neighbor->has_children());
                ...
                for (unsigned int c=0; c!=neighbor->n_children(); c++)
                {
                  ...
                  if (subneighbor->active() &&
                        ...
                        subneighbor->p_refinement_flag() != Elem::REFINE)
                    {
                      ...
                      // compatible
                      genius_assert(subneighbor->p_level() + 2u >
                             my_p_level);
...
bool MeshRefinement::make_interface_compatible ()
{
  ...
  for (; el != end_el; ++el)
  {
    ...
    {
      Elem* neighbor = elem->neighbor(side);
      // I have a neighbor on interface and it is active
      if (elem->on_interface(side) && neighbor->active() )
      {
        assert (neighbor->level() == my_level );
        // we make sure for the interface element:
	// The neighbor is at the same level as elem am for ever.
        // since make the same action as elem
        if ( elem->refinement_flag() != neighbor->refinement_flag() )
	{
          neighbor->set_refinement_flag(elem->refinement_flag());
          level_one_satisfied = false;
        }
      }
    }
