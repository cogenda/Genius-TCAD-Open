...
void SimpleGateContactBC::_find_mos_channel_elem()
{
  ...
  for(unsigned int n=0; n<el.size(); ++n)
  {
    ...
    while(!queue.empty())
    {
      ...
      queue.pop();
      visited_elem.insert(current_elem);
      ...
      for(unsigned int n=0; n<current_elem->n_sides(); ++n)
      {
        const Elem * neighbor_elem = current_elem->neighbor(n);
        if( neighbor_elem && visited_elem.find(neighbor_elem) == visited_elem.end() )
        {
  ...
  //std::cout<<semiconductor_region->name() << " " << semiconductor_region->_elem_in_mos_channel.size()<<std::endl;
}
