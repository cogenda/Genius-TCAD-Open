  ...
  for(; node_it!=end_it; ++node_it )
  {
    ...
    for(unsigned int i=0 ; rnode_it!=end_rnode_it; ++i, ++rnode_it  )
    {
      ...
      else
      {
        ...
        // current flow
        VecSetValue(f, fvm_nodes[i]->global_offset()+node_n_offset,  Jn*cv_boundary, ADD_VALUES);
        VecSetValue(f, fvm_nodes[0]->global_offset()+node_n_offset, -Jn*cv_boundary, ADD_VALUES);

        VecSetValue(f, fvm_nodes[i]->global_offset()+node_p_offset, -Jp*cv_boundary, ADD_VALUES);
        VecSetValue(f, fvm_nodes[0]->global_offset()+node_p_offset,  Jp*cv_boundary, ADD_VALUES);
        ...
        if(regions[i]->get_advanced_model()->enable_Tn())
          VecSetValue(f, fvm_nodes[i]->global_offset()+node_Tn_offset,  -Sn*cv_boundary, ADD_VALUES);
        if(regions[0]->get_advanced_model()->enable_Tn())
          VecSetValue(f, fvm_nodes[0]->global_offset()+regions[0]->ebm_variable_offset(E_TEMP), Sn*cv_boundary, ADD_VALUES);


        // hole energy flux
        if(regions[i]->get_advanced_model()->enable_Tp())
          VecSetValue(f, fvm_nodes[i]->global_offset()+node_Tp_offset,  -Sp*cv_boundary, ADD_VALUES);
        if(regions[0]->get_advanced_model()->enable_Tp())
          VecSetValue(f, fvm_nodes[0]->global_offset()+regions[0]->ebm_variable_offset(H_TEMP), Sp*cv_boundary, ADD_VALUES);
      }
    }
  ...
  for(node_it = nodes_begin(); node_it!=end_it; ++node_it )
  {
    ...
    for(unsigned int i=0 ; rnode_it!=end_rnode_it; ++i, ++rnode_it  )
    {
      ...
      if(i==0)
      {
        ...
        V0 = x[fvm_nodes[i]->local_offset()+node_psi_offset];  V0.setADValue(node_psi_offset,1.0);
        n0 = x[fvm_nodes[i]->local_offset()+node_n_offset];    n0.setADValue(node_n_offset,1.0);  // electron density
        p0 = x[fvm_nodes[i]->local_offset()+node_p_offset];    p0.setADValue(node_p_offset,1.0);  // hole density
      ...
      else
      {
        ...
        AutoDScalar V = x[fvm_nodes[i]->local_offset()+node_psi_offset];  V.setADValue(n_node_var_0+node_psi_offset, 1.0); // psi of this node
        AutoDScalar n = x[fvm_nodes[i]->local_offset()+node_n_offset];    n.setADValue(n_node_var_0+node_n_offset, 1.0);   // electron density
        AutoDScalar p = x[fvm_nodes[i]->local_offset()+node_p_offset];    p.setADValue(n_node_var_0+node_p_offset, 1.0);   // hole density
        ...
        // current flow
        MatSetValues(*jac, 1, &rows[n_node_var_0+node_n_offset], cols.size(), &cols[0], (Jn*cv_boundary).getADValue(), ADD_VALUES);
        MatSetValues(*jac, 1, &rows[node_n_offset], cols.size(), &cols[0], (-Jn*cv_boundary).getADValue(), ADD_VALUES);

        MatSetValues(*jac, 1, &rows[n_node_var_0+node_p_offset], cols.size(), &cols[0], (-Jp*cv_boundary).getADValue(), ADD_VALUES);
        MatSetValues(*jac, 1, &rows[node_p_offset], cols.size(), &cols[0], (Jp*cv_boundary).getADValue(), ADD_VALUES);

        // electron energy flux
        if(regions[i]->get_advanced_model()->enable_Tn())
          MatSetValues(*jac, 1, &rows[n_node_var_0+node_Tn_offset], cols.size(), &cols[0], (-Sn*cv_boundary).getADValue(), ADD_VALUES);
        if(regions[0]->get_advanced_model()->enable_Tn())
          MatSetValues(*jac, 1, &rows[node_Tn_offset], cols.size(), &cols[0], (Sn*cv_boundary).getADValue(), ADD_VALUES);


        // hole energy flux
        if(regions[i]->get_advanced_model()->enable_Tp())
          MatSetValues(*jac, 1, &rows[n_node_var_0+node_Tp_offset], cols.size(), &cols[0], (-Sp*cv_boundary).getADValue(), ADD_VALUES);
        if(regions[0]->get_advanced_model()->enable_Tp())
          MatSetValues(*jac, 1, &rows[node_Tp_offset], cols.size(), &cols[0], (Sp*cv_boundary).getADValue(), ADD_VALUES);
