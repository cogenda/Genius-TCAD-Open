...
/*---------------------------------------------------------------------
 * do pre-process to function evaluation for EBM3 solver
 */
void ElectrodeInsulatorInterfaceBC::EBM3_Function_Preprocess(Vec, std::vector<PetscInt> &src_row,
 ...
 */
void ElectrodeInsulatorInterfaceBC::EBM3_Jacobian_Preprocess(Mat *jac, std::vector<PetscInt> &src_row,
    std::vector<PetscInt> &dst_row, std::vector<PetscInt> &clear_row)
{
 // search for all the node with this boundary type
  BoundaryCondition::const_node_iterator node_it = nodes_begin();
