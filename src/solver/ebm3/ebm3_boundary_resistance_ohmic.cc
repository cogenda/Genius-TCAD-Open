...
void IF_Metal_OhmicBC::EBM3_Jacobian( PetscScalar * x, Mat *jac, InsertMode &add_value_flag )
{
...
void IF_Metal_OhmicBC::_EBM3_Jacobian_Infinite_Recombination ( PetscScalar * x, Mat *jac, InsertMode &add_value_flag )
{
  ...
  {
    MatSetValues(*jac, 1, &_buffer_rows[n], _buffer_cols[n].size(), &(_buffer_cols[n])[0], &(_buffer_jacobian_entries[n])[0], ADD_VALUES);
  }
  ...
  for ( ; node_it!=end_it; ++node_it )
  {
    ...
    MatSetValues(*jac, 1, &gloabl_node_n_offset, col.size(), &col[0], f_elec.getADValue(), ADD_VALUES);
    MatSetValues(*jac, 1, &gloabl_node_p_offset, col.size(), &col[0], f_hole.getADValue(), ADD_VALUES);
 ...
 */
void IF_Metal_OhmicBC::EBM3_Jacobian_Reserve ( Mat *jac, InsertMode &add_value_flag )
{
