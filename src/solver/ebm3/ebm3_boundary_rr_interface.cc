 ...
 */
void ResistanceResistanceBC::EBM3_Fill_Value(Vec , Vec L)
{
 ...
 */
void ResistanceResistanceBC::EBM3_Function_Preprocess(PetscScalar *,Vec f, std::vector<PetscInt> &src_row,
    std::vector<PetscInt> &dst_row, std::vector<PetscInt> &clear_row)
{
  ...
  for(; node_it!=end_it; ++node_it )
  {
    ...
    if(_r1->get_advanced_model()->enable_Tl())
    {
 ...
 */
void ResistanceResistanceBC::EBM3_Function(PetscScalar * x, Vec f, InsertMode &add_value_flag)
{
  ...
  for(; node_it!=end_it; ++node_it )
  {
    ...
    if ( has_associated_region ( ( *node_it ), InsulatorRegion ) )
    {
      ...
      for ( ; rnode_it!=end_rnode_it; ++rnode_it )
      {
        ...
        if(region->get_advanced_model()->enable_Tl())
        {
          const PetscScalar T_resistance_1 = x[resistance_fvm_node_1->local_offset()+1];
 ...
 */
void ResistanceResistanceBC::EBM3_Jacobian_Reserve(Mat *jac, InsertMode &add_value_flag)
{
  ...
  for(; node_it!=end_it; ++node_it )
  {
    ...
    for(unsigned int i=0 ; rnode_it!=end_rnode_it; ++i, ++rnode_it  )
    {
      ...
      MatSetValue(*jac, insulator_fvm_node->global_offset()+0, resistance_fvm_node_1->global_offset()+0, 0, ADD_VALUES);
      MatSetValue(*jac, insulator_fvm_node->global_offset()+0, resistance_fvm_node_2->global_offset()+0, 0, ADD_VALUES);

      MatSetValue(*jac, resistance_fvm_node_1->global_offset()+0, insulator_fvm_node->global_offset()+0, 0, ADD_VALUES);
      FVM_Node::fvm_neighbor_node_iterator nb_it = insulator_fvm_node->neighbor_node_begin();
      for(; nb_it != insulator_fvm_node->neighbor_node_end(); ++nb_it)
      {
        const FVM_Node *nb_node = (*nb_it).first;
        MatSetValue(*jac, resistance_fvm_node_1->global_offset()+0, nb_node->global_offset()+0, 0, ADD_VALUES);
      }

      if(region->get_advanced_model()->enable_Tl())
      {
        MatSetValue(*jac, insulator_fvm_node->global_offset()+1, resistance_fvm_node_1->global_offset()+1, 0, ADD_VALUES);
        MatSetValue(*jac, resistance_fvm_node_1->global_offset()+1, insulator_fvm_node->global_offset()+1, 0, ADD_VALUES);
        FVM_Node::fvm_neighbor_node_iterator nb_it = insulator_fvm_node->neighbor_node_begin();
        for(; nb_it != insulator_fvm_node->neighbor_node_end(); ++nb_it)
        {
          const FVM_Node *nb_node = (*nb_it).first;
          MatSetValue(*jac, resistance_fvm_node_1->global_offset()+1, nb_node->global_offset()+1, 0, ADD_VALUES);
        }
      }
    }
  }
 ...
 */
void ResistanceResistanceBC::EBM3_Jacobian_Preprocess(PetscScalar * ,Mat *jac, std::vector<PetscInt> &src_row,
    std::vector<PetscInt> &dst_row, std::vector<PetscInt> &clear_row)
{
  const SimulationRegion * _r1 = bc_regions().first;
  const SimulationRegion * _r2 = bc_regions().second;
 ...
 * build function and its jacobian for EBM3 solver
 */
void ResistanceResistanceBC::EBM3_Jacobian(PetscScalar * x, Mat *jac, InsertMode &add_value_flag)
{
  // since we will use ADD_VALUES operat, check the matrix state.
  if( (add_value_flag != ADD_VALUES) && (add_value_flag != NOT_SET_VALUES) )
  {
  ...
  for(node_it = nodes_begin(); node_it!=end_it; ++node_it )
  {
    ...
    if(_r1->get_advanced_model()->enable_Tl())
    {
      ...
      MatSetValue(*jac, resistance_fvm_node_2->global_offset()+1, resistance_fvm_node_1->global_offset()+1, f_T.getADValue(0), ADD_VALUES);
      MatSetValue(*jac, resistance_fvm_node_2->global_offset()+1, resistance_fvm_node_2->global_offset()+1, f_T.getADValue(1), ADD_VALUES);
    }
    ...
    if ( has_associated_region ( ( *node_it ), InsulatorRegion ) )
    {
      ...
      for ( ; rnode_it!=end_rnode_it; ++rnode_it )
      {
        ...
        {
          AutoDScalar T_resistance_1 = x[resistance_fvm_node_1->local_offset()+1]; T_resistance_1.setADValue(0,1.0);
          AutoDScalar T_insulator  = x[insulator_fvm_node->local_offset()+1]; T_insulator.setADValue(1, 1.0);
          // the T of this node is equal to corresponding T of  node in the first resistance region
          // since T should be continuous for the interface
          AutoDScalar f_T = T_insulator - T_resistance_1;

          // set Jacobian of governing equation f_T
          MatSetValue(*jac, insulator_fvm_node->global_offset()+1, resistance_fvm_node_1->global_offset()+1, f_T.getADValue(0), ADD_VALUES);
        ...
        if(SolverSpecify::TimeDependent == true)
        {
          ...
          for(; nb_it != insulator_fvm_node->neighbor_node_end(); ++nb_it)
          {
            ...
            MatSetValue(*jac, resistance_fvm_node_1->global_offset(), insulator_fvm_node->global_offset(), -current_disp.getADValue(0), ADD_VALUES);
            MatSetValue(*jac, resistance_fvm_node_1->global_offset(), nb_node->global_offset(), -current_disp.getADValue(1), ADD_VALUES);
          }
        }
      }
    }
