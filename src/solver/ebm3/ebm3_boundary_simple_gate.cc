 ...
 */
void SimpleGateContactBC::EBM3_Function(PetscScalar * x, Vec f, InsertMode &add_value_flag)
{
  ...
  for(; node_it!=end_it; ++node_it )
  {
    ...
    if( region->get_advanced_model()->enable_Tl() && (node_on_boundary(*node_it) || has_associated_region(*node_it, VacuumRegion)) )
    {
      ...
      PetscScalar T = x[fvm_node->local_offset()+node_Tl_offset]; // T of this node
