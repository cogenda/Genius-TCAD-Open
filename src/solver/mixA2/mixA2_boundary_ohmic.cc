 ...
 */
void OhmicContactBC::MixA_DDM2_Jacobian_Preprocess(PetscScalar *,SparseMatrix<PetscScalar> *jac, std::vector<PetscInt> &src_row,  std::vector<PetscInt> &dst_row, std::vector<PetscInt> &clear_row)
{
 ...
 * build function and its jacobian for DDML2 solver
 */
void OhmicContactBC::MixA_DDM2_Jacobian(PetscScalar * x, SparseMatrix<PetscScalar> *jac, InsertMode &add_value_flag)
{
  ...
  {
    jac->add_row(  bc_global_offset,  _buffer_cols[n].size(),  &(_buffer_cols[n])[0],  &(_buffer_jacobian_entries[n])[0] );
  }
    ...
    for(unsigned int i=0 ; rnode_it!=end_rnode_it; ++i, ++rnode_it  )
    {
      ...
      switch ( regions[i]->type() )
      {
          ...
          case SemiconductorRegion:
          {
            ...
            jac->add_row(  row[0],  col.size(),  &col[0],  ff1.getADValue() );
            jac->add_row(  row[1],  col.size(),  &col[0],  ff2.getADValue() );
            jac->add_row(  row[2],  col.size(),  &col[0],  ff3.getADValue() );
 ...
 * update electrode IV
 */
void OhmicContactBC::MixA_DDM2_Update_Solution(PetscScalar *)
{
