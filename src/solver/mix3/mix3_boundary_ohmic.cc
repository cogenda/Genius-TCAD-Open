 ...
 */
void OhmicContactBC::Mix_EBM3_Function(PetscScalar * x, Vec f, InsertMode &add_value_flag)
{
 ...
 */
void OhmicContactBC::Mix_EBM3_Jacobian_Reserve(Mat *jac, InsertMode &add_value_flag)
{
 ...
 * build function and its jacobian for Mixed EBM3 solver
 */
void OhmicContactBC::Mix_EBM3_Jacobian(PetscScalar * x, Mat *jac, InsertMode &add_value_flag)
{
  ...
  // we  zero all the rows corresponding to Ohmic bc since they have been filled previously.
  {
        ...
        case SemiconductorRegion:
          {
            ...
            rm_row.push_back(row+regions[i]->ebm_variable_offset(POTENTIAL));
            rm_row.push_back(row+regions[i]->ebm_variable_offset(ELECTRON));
            rm_row.push_back(row+regions[i]->ebm_variable_offset(HOLE));

            if(regions[i]->get_advanced_model()->enable_Tn())
              rm_row.push_back(row+regions[i]->ebm_variable_offset(E_TEMP));
            if(regions[i]->get_advanced_model()->enable_Tp())
              rm_row.push_back(row+regions[i]->ebm_variable_offset(H_TEMP));
            break;
          }
        case ConductorRegion:
        case InsulatorRegion:
          {
            ...
            if(regions[i]->get_advanced_model()->enable_Tl())
            {
              ...
              rm_row.push_back(row+regions[i]->ebm_variable_offset(TEMPERATURE));
            }
      ...
      case SemiconductorRegion:
        {
          ...
          //the indepedent variable number, we only need n_node_var here.
...
void OhmicContactBC::Mix_EBM3_Electrode_Load(const Vec , const Mat * jac, double & I, PetscScalar & pdI_pdV, Vec & pdI_pdx, Vec & pdF_pdV)
{
