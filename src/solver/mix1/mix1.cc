...
int Mix1Solver::solve()
{
  ...
  START_LOG("Mix1Solver_SNES()", "Mix1Solver");

  run_under_ngspice();

  STOP_LOG("Mix1Solver_SNES()", "Mix1Solver");
 ...
 */
int Mix1Solver::get_electrode_load()
{
  ...
  {
    SimulationRegion * region = _system.region(n);
    region->DDM1_Jacobian(lxx, &G, add_value_flag);
  }
  ...
  // before first assemble, resereve none zero pattern for each boundary
  {
    if( !g_matrix_first_assemble )
      for(unsigned int b=0; b<_system.get_bcs()->n_bcs(); b++)
      {
        ...
        bc->Mix_DDM1_Jacobian_Reserve(&G, add_value_flag);
      }
...
#endif
  {
    ...
    for(unsigned int b=0; b<_system.get_bcs()->n_bcs(); b++)
    {
      ...
      bc->Mix_DDM1_Electrode_Load(lx, &G, PINinfos[pin].I, PINconds[pin].pdI_pdV, PINconds[pin].pdI_pdw, PINconds[pin].pdF_pdV );
  ...
  if(SolverSpecify::TimeDependent == true)
    ...
    {
      SimulationRegion * region = _system.region(n);
      region->DDM1_Time_Dependent_Jacobian(lxx, &G, add_value_flag);
    }


  // evaluate Jacobian matrix of governing equations of Mixed type simulation of DDML1 for all the boundaries
  for(unsigned int b=0; b<_system.get_bcs()->n_bcs(); b++)
  {
    BoundaryCondition * bc = _system.get_bcs()->get_bc(b);
    bc->Mix_DDM1_Jacobian(lxx, &G, add_value_flag);
  }
 ...
 */
int Mix1Solver::init_spice_data()
{
 ...
 */
int Mix1Solver::load_spice_data()
{
 ...
 */
int Mix1Solver::save_spice_data()
{
 ...
 */
void Mix1Solver::potential_damping(Vec x, Vec y, Vec w, PetscTruth *changed_y, PetscTruth *changed_w)
{
 ...
 */
void Mix1Solver::bank_rose_damping(Vec , Vec , Vec , PetscTruth *changed_y, PetscTruth *changed_w)
{
  *changed_y = PETSC_FALSE;
 ...
 */
void Mix1Solver::positive_density_damping(Vec x, Vec y, Vec w, PetscTruth *changed_y, PetscTruth *changed_w)
{
...
void Mix1Solver::build_petsc_sens_residual(Vec x, Vec r)
{
  ...
  // evaluate governing equations of Mixed type simulation of DDML1 for all the boundaries
  for(unsigned int b=0; b<_system.get_bcs()->n_bcs(); b++)
  {
    BoundaryCondition * bc = _system.get_bcs()->get_bc(b);
