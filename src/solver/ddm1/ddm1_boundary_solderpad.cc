 ...
 */
void SolderPadBC::DDM1_Jacobian_Preprocess(PetscScalar *, SparseMatrix<PetscScalar> *jac, std::vector<PetscInt> &src_row,
    std::vector<PetscInt> &dst_row, std::vector<PetscInt> &clear_row)
{
  BoundaryCondition::const_node_iterator node_it = nodes_begin();
  BoundaryCondition::const_node_iterator end_it = nodes_end();
 ...
 * build function and its jacobian for DDM L1 solver
 */
void SolderPadBC::DDM1_Jacobian(PetscScalar * x, SparseMatrix<PetscScalar> *jac, InsertMode &add_value_flag)
{
...
void SolderPadBC::DDM1_Electrode_Trace(Vec lx, SparseMatrix<PetscScalar> *jac, Vec pdI_pdx, Vec pdF_pdV)
{
  ...
  for(node_it = nodes_begin(); node_it!=end_it; ++node_it )
  {
    ...
    for ( ; rnode_it!=end_rnode_it; ++rnode_it )
    {
      ...
      for(; nb_it != nb_it_end; ++nb_it)
      {
        ...
        AutoDScalar current_density = resistance_region->material()->basic->CurrentDensity((V-Vn)/distance, T);
