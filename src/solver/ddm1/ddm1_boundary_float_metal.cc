 ...
 */
void ChargedContactBC::DDM1_Function_Preprocess(PetscScalar *, Vec f, std::vector<PetscInt> &src_row,
...
/*---------------------------------------------------------------------
 * do pre-process to jacobian matrix for DDM1 solver
 */
void ChargedContactBC::DDM1_Jacobian_Preprocess(PetscScalar *, SparseMatrix<PetscScalar> *jac, std::vector<PetscInt> &src_row,
    std::vector<PetscInt> &dst_row, std::vector<PetscInt> &clear_row)
{
 ...
 * build function and its jacobian for DDM1 solver
 */
void ChargedContactBC::DDM1_Jacobian(PetscScalar * x, SparseMatrix<PetscScalar> *jac, InsertMode &add_value_flag)
{
...
void ChargedContactBC::DDM1_Pre_Process()
{
  _current_flow = 0.0;
}
