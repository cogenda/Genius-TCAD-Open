 ...
 */
void OhmicContactBC::DDM1_Jacobian_Reserve(Mat *jac, InsertMode &add_value_flag)
{
 ...
 * build function and its jacobian for DDML1 solver
 */
void OhmicContactBC::DDM1_Jacobian(PetscScalar * x, Mat *jac, InsertMode &add_value_flag)
{
  ...
  // which is required by external electrode circuit
  {
    ...
    for(node_it = nodes_begin(); node_it!=end_it; ++node_it )
    {
      ...
      for(; nb_it != nb_it_end; ++nb_it)
      {
        ...
        if(this->is_inter_connect_electrode())
        {
          ...
          // displacement current
          JM[0]+= R*cv_boundary*node_data->eps()/distance*disp_t*current_scale;
          JN[0]-= R*cv_boundary*node_data->eps()/distance*disp_t*current_scale;
        }
        // for stand alone electrode
        else
        {
          if(ext_circuit()->is_voltage_driven())
          {
            ...
            // displacement current
            JM[0]+= (L/dt+R)*cv_boundary*node_data->eps()/distance*disp_t*current_scale;
            JN[0]-= (L/dt+R)*cv_boundary*node_data->eps()/distance*disp_t*current_scale;
          }
    ...
    {
      MatSetValues(*jac, 1, &bc_global_offset, 3, &(cols[n])[0], &(current_jacobian[n])[0], ADD_VALUES);
    }
  }
  ...
  // then, we can zero all the rows corresponding to Ohmic bc since they have been filled previously.
  {
    ...
    //no matter whether the processor owns this row!
      ...
      case SemiconductorRegion:
        {
          ...
          MatSetValues(*jac, 1, &row[0], col.size(), &col[0], ff1.getADValue(), INSERT_VALUES);
          MatSetValues(*jac, 1, &row[1], col.size(), &col[0], ff2.getADValue(), INSERT_VALUES);
          MatSetValues(*jac, 1, &row[2], col.size(), &col[0], ff3.getADValue(), INSERT_VALUES);
...
void OhmicContactBC::DDM1_Electrode_Trace(Vec, Mat *jac, Vec pdI_pdx, Vec pdF_pdV)
{
  ...
  for(node_it = nodes_begin(); node_it!=end_it; ++node_it )
  {
    ...
    PetscScalar A1[3], A2[3], JM[3], JN[3];
    PetscInt    row[3];
    row[0] = fvm_node->global_offset()+0;
    row[1] = fvm_node->global_offset()+1;
    row[2] = fvm_node->global_offset()+2;
    ...
    for(; nb_it != nb_it_end; ++nb_it)
    {
      ...
      VecSetValues( pdI_pdx, 3, &col[0], &JN[0], ADD_VALUES);
    }

    VecSetValues( pdI_pdx, 3, &row[0], &JM[0], ADD_VALUES);
