 ...
 * fill scaling value
 */
void SolderPadBC::MixA_DDM1_Fill_Value(Vec x, Vec L)
{
 ...
 */
void SolderPadBC::MixA_DDM1_Function(PetscScalar * x, Vec f, InsertMode &add_value_flag)
{
      ...
      case MetalRegion :
        {
          ...
          for(; nb_it != fvm_node->neighbor_node_end(); ++nb_it)
          {
            ...
            // current density 
            PetscScalar current_density = resistance_region->material()->basic->CurrentDensity((V-V_nb)/distance, T);
            // current flow
 ...
 */
void SolderPadBC::MixA_DDM1_Jacobian_Preprocess(PetscScalar *, SparseMatrix<PetscScalar> *jac, std::vector<PetscInt> &src_row,
    std::vector<PetscInt> &dst_row, std::vector<PetscInt> &clear_row)
{
  BoundaryCondition::const_node_iterator node_it = nodes_begin();
  BoundaryCondition::const_node_iterator end_it = nodes_end();
 ...
 * build function and its jacobian for Mixed DDML1 solver
 */
void SolderPadBC::MixA_DDM1_Jacobian(PetscScalar * x, SparseMatrix<PetscScalar> *jac, InsertMode &add_value_flag)
{
      ...
      case MetalRegion :
        {
          ...
          for(; nb_it != fvm_node->neighbor_node_end(); ++nb_it)
          {
            ...
            // current density
            AutoDScalar current_density = resistance_region->material()->basic->CurrentDensity((V-V_nb)/distance, T);
            ...
            jac->add( bc_global_offset,  nb_node->global_offset(),   current.getADValue(1) );
          }
          break;
        }
 ...
 * update electrode IV
 */
void SolderPadBC::MixA_DDM1_Update_Solution(PetscScalar *)
{
