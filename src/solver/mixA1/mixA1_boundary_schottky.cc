 ...
 */
void SchottkyContactBC::MixA_DDM1_Function_Preprocess(PetscScalar *, Vec f, std::vector<PetscInt> &src_row,
 ...
 */
void SchottkyContactBC::MixA_DDM1_Function(PetscScalar * x, Vec f, InsertMode &add_value_flag)
{
 ...
 */
void SchottkyContactBC::MixA_DDM1_Jacobian_Reserve(Mat *jac, InsertMode &add_value_flag)
{
 ...
 */
void SchottkyContactBC::MixA_DDM1_Jacobian_Preprocess(PetscScalar *, Mat *jac, std::vector<PetscInt> &src_row,
    std::vector<PetscInt> &dst_row, std::vector<PetscInt> &clear_row)
{
  BoundaryCondition::const_node_iterator node_it = nodes_begin();
  BoundaryCondition::const_node_iterator end_it = nodes_end();
 ...
 * build function and its jacobian for Mixed DDML1 solver
 */
void SchottkyContactBC::MixA_DDM1_Jacobian(PetscScalar * x, Mat *jac, InsertMode &add_value_flag)
{
    ...
    for(unsigned int i=0 ; rnode_it!=end_rnode_it; ++i, ++rnode_it  )
    {
      ...
      switch ( regions[i]->type() )
      {
          ...
          case SemiconductorRegion:
          {
            ...
            MatSetValues(*jac, 1, &bc_global_offset, col.size(), &(col[0]), (current_emit*current_scale).getADValue(), ADD_VALUES);
