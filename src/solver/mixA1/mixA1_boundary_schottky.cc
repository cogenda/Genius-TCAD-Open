 ...
 */
void SchottkyContactBC::MixA_DDM1_Jacobian_Preprocess(PetscScalar *, SparseMatrix<PetscScalar> *jac, std::vector<PetscInt> &src_row,
    std::vector<PetscInt> &dst_row, std::vector<PetscInt> &clear_row)
{
  BoundaryCondition::const_node_iterator node_it = nodes_begin();
  BoundaryCondition::const_node_iterator end_it = nodes_end();
 ...
 * build function and its jacobian for Mixed DDML1 solver
 */
void SchottkyContactBC::MixA_DDM1_Jacobian(PetscScalar * x, SparseMatrix<PetscScalar> *jac, InsertMode &add_value_flag)
{
 ...
 * update electrode IV
 */
void SchottkyContactBC::MixA_DDM1_Update_Solution(PetscScalar *)
{
