        ...
        if(bc->is_electrode())
        {
            ...
            for(unsigned int n=0; n<bc_nodes.size(); ++n  )
            {
                if( bc_nodes[n]->processor_id() == Genius::processor_id() )
                    ...
                    off_processor_dofs += (neighbors[n]+1)*this->n_bc_node_dofs( bc ) ;
            }
...
int MixASolverBase::solve_dcop()
{
    ...
    if(Genius::is_last_processor())
    {
        ...
        {
            double idc =  _circuit->get_voltage_from(isrcs[n]);
            isrc_dc[isrcs[n]]= idc;
...
int MixASolverBase::solve_transient()
{
    ...
    Parallel::broadcast(uic, Genius::last_processor_id());
    ...
    {
        // get opreator point
        MESSAGE<<"Compute operator point for transient simulation\n";
        RECORD();
        ...
        _system.get_field_source()->update(SolverSpecify::TStart);
        ...
        if(reason<0)
        {
            MESSAGE
            <<"-----------------------------------------------------------------------------\n"
            <<"------> nonlinear solver "<<SNESConvergedReasons[reason]<<", Transient solution failed.\n\n\n";
            RECORD();
            goto end;
        }
        else
