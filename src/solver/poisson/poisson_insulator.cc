...
//  $Id: poisson_insulator.cc,v 1.11 2008/07/09 05:58:16 gdiso Exp $
...
void InsulatorSimulationRegion::Poissin_Fill_Value(Vec x, Vec L)
{
 ...
 */
void InsulatorSimulationRegion::Poissin_Function(PetscScalar * x, Vec f, InsertMode &add_value_flag)
{
  ...
  for(; it!=it_end; ++it)
  {
    // for insulator, \rho is always zero (right?) . no need to process it.
    ...
    for(unsigned int ne=0; ne<(*it)->n_edges(); ++ne )
    {
      ...
      // build nonlinear poisson's equation
      {
        ...
        PetscScalar eps1 =  n1_data->eps();     // electrostatic potential of node1


        PetscScalar V2   =  x[n2_local_offset]; // electrostatic potential of node2
        PetscScalar eps2 =  n2_data->eps();     // electrostatic potential of node2
...
void InsulatorSimulationRegion::Poissin_Update_Solution(PetscScalar *lxx)
{
