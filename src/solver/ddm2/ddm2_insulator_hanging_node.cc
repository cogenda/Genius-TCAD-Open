...
void InsulatorSimulationRegion::DDM2_Function_Hanging_Node(PetscScalar *x, Vec f, InsertMode &add_value_flag)
{
  ...
  // process hanging node lies on side center
  {
	...
	alpha_buffer.push_back(1.0/n_side_node);
      ...
      PetscScalar T1 = x[interpolation_p1->local_offset()+1];

      PetscScalar V2 = x[interpolation_p2->local_offset()+0];
      PetscScalar T2 = x[interpolation_p2->local_offset()+1];
  ...
  // process hanging node lies on edge center
  {
	...
	alpha_buffer.push_back(1.0/n_edge_node);
      ...
      PetscScalar T1 = x[edge_fvm_nodes[0]->local_offset()+1];

      PetscScalar V2 = x[edge_fvm_nodes[1]->local_offset()+0];
      PetscScalar T2 = x[edge_fvm_nodes[1]->local_offset()+1];
...
void InsulatorSimulationRegion::DDM2_Jacobian_Hanging_Node(PetscScalar *x, Mat *jac, InsertMode &add_value_flag)
{
  ...
  // process hanging node lies on side center
  {
	...
	alpha_buffer.push_back(1.0/n_side_node);
      ...
      AutoDScalar T1 = x[interpolation_p1->local_offset()+1]; T1.setADValue(1, 1.0);
      AutoDScalar T2 = x[interpolation_p2->local_offset()+1]; T2.setADValue(2, 1.0);

      cols[0] = fvm_node->global_offset()+1;
      cols[1] = interpolation_p1->global_offset()+1;
      cols[2] = interpolation_p2->global_offset()+1;
  ...
  // process hanging node lies on edge center
  {
	...
	alpha_buffer.push_back(1.0/n_edge_node);
      ...
      // the lattice temperature is also linear interpolated
      AutoDScalar T = x[fvm_node->local_offset()+1];           T.setADValue(0, 1.0);
      AutoDScalar T1 = x[edge_fvm_nodes[0]->local_offset()+1]; T1.setADValue(1, 1.0);
      AutoDScalar T2 = x[edge_fvm_nodes[1]->local_offset()+1]; T2.setADValue(2, 1.0);

      cols[0] = fvm_node->global_offset()+1;
      cols[1] = edge_fvm_nodes[0]->global_offset()+1;
      cols[2] = edge_fvm_nodes[1]->global_offset()+1;
