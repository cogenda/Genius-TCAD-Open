 ...
 */
void HeteroInterfaceBC::DDM2_Jacobian_Preprocess(PetscScalar *,SparseMatrix<PetscScalar> *jac, std::vector<PetscInt> &src_row,
    std::vector<PetscInt> &dst_row, std::vector<PetscInt> &clear_row)
{
  // search for all the node with this boundary type
  BoundaryCondition::const_node_iterator node_it = nodes_begin();
 ...
 * build function and its jacobian for DDML2 solver
 */
void HeteroInterfaceBC::DDM2_Jacobian(PetscScalar * x, SparseMatrix<PetscScalar> *jac, InsertMode &add_value_flag)
{
      ...
      else
      {
        ...
        switch( region->type() )
        {
            case SemiconductorRegion :
            {
              ...
              jac->add_row(  rows[4],  cols.size(),  &cols[0],  ff1.getADValue() );

              AutoDScalar ff4 = T - T0;
              jac->add_row(  rows[7],  cols.size(),  &cols[0],  ff4.getADValue() );
              ...
              if(Ec0 > Ec)
              {
                ...
                jac->add_row(  rows[5],  cols.size(),  &cols[0],  Jn.getADValue() );
              ...
              if(Ev0 < Ev)
              {
                ...
                jac->add_row(  rows[6],  cols.size(),  &cols[0],  Jp.getADValue() );
