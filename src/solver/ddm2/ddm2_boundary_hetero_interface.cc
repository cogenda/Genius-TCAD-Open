    ...
    for(unsigned int i=0 ; rnode_it!=end_rnode_it; ++i, ++rnode_it  )
    {
...

      {
        ...
        for(; gn_it != fvm_nodes[i]->ghost_node_end(); ++gn_it)
        {
          ...
          cols.push_back(ghost_fvm_node->global_offset()+3);
          ...
          for(; gnb_it != ghost_fvm_node->neighbor_node_end(); ++gnb_it)
          {
            ...
            cols.push_back((*gnb_it).second->global_offset()+3);
          }
        }
  ...
  for(node_it = nodes_begin(); node_it!=end_it; ++node_it )
  {
    ...
    for(unsigned int i=0 ; rnode_it!=end_rnode_it; ++i, ++rnode_it  )
    {
      ...
      else
      {
        ...
        MatSetValues(*jac, 1, &rows[5], cols.size(), &cols[0], Fn.getADValue(), ADD_VALUES);
        MatSetValues(*jac, 1, &rows[1], cols.size(), &cols[0], (-Fn).getADValue(), ADD_VALUES);

        AutoDScalar Fp = -Jp*cv_boundary;
        MatSetValues(*jac, 1, &rows[6], cols.size(), &cols[0], Fp.getADValue(), ADD_VALUES);
