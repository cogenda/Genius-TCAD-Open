 ...
 */
void PolyGateContactBC::DDM2_Fill_Value(Vec x, Vec L)
{
 ...
 */
void PolyGateContactBC::DDM2_Function_Preprocess(PetscScalar * ,Vec f, std::vector<PetscInt> &src_row,
  ...
  for(; node_it!=end_it; ++node_it )
  {
    ...
    for(unsigned int i=0 ; rnode_it!=end_rnode_it; ++i, ++rnode_it  )
    {
      ...
      case SemiconductorRegion:
        {
          PetscInt row = fvm_nodes[0]->global_offset();
 ...
 */
void PolyGateContactBC::DDM2_Function(PetscScalar * x, Vec f, InsertMode &add_value_flag)
{
      ...
      case InsulatorRegion:
        {
          ...
          PetscScalar T_0 = x[fvm_nodes[0]->local_offset()+3];

          // the psi of this node is equal to corresponding psi of semiconductor node
          PetscScalar f_psi = V - V_0;
          PetscScalar f_T = T - T_0;

          // set governing equation to function vector
          VecSetValue(f, fvm_nodes[i]->global_offset(), f_psi, ADD_VALUES);
          VecSetValue(f, fvm_nodes[i]->global_offset()+1, f_T, ADD_VALUES);
 ...
 */
void PolyGateContactBC::DDM2_Jacobian_Preprocess(PetscScalar *,SparseMatrix<PetscScalar> *jac, std::vector<PetscInt> &src_row,
    std::vector<PetscInt> &dst_row, std::vector<PetscInt> &clear_row)
{
  BoundaryCondition::const_node_iterator node_it = nodes_begin();
  BoundaryCondition::const_node_iterator end_it = nodes_end();
 ...
 * build function and its jacobian for DDML2 solver
 */
void PolyGateContactBC::DDM2_Jacobian(PetscScalar * x, SparseMatrix<PetscScalar> *jac, InsertMode &add_value_flag)
{
      ...
      case SemiconductorRegion:
        {
          ...
          AutoDScalar  T = x[fvm_nodes[i]->local_offset()+3]; T.setADValue(1, 1.0);
          ...
          jac->add( fvm_nodes[i]->global_offset(),  this->global_offset(),  f_psi.getADValue(2) );

          jac->add( fvm_nodes[i]->global_offset()+1,  fvm_nodes[i]->global_offset()+1,  fn.getADValue(0) );
          jac->add( fvm_nodes[i]->global_offset()+1,  fvm_nodes[i]->global_offset()+3,  fn.getADValue(1) );

          jac->add( fvm_nodes[i]->global_offset()+2,  fvm_nodes[i]->global_offset()+2,  fp.getADValue(0) );
          jac->add( fvm_nodes[i]->global_offset()+2,  fvm_nodes[i]->global_offset()+3,  fp.getADValue(1) );
      ...
      case InsulatorRegion:
        {
          ...
          // the psi of this node is equal to corresponding psi of semiconductor node
          AutoDScalar  f_psi = V - V_0;

          // set Jacobian of governing equation f_psi
          jac->add( fvm_nodes[i]->global_offset(),  fvm_nodes[i]->global_offset(),  f_psi.getADValue(0) );
          jac->add( fvm_nodes[i]->global_offset(),  fvm_nodes[0]->global_offset(),  f_psi.getADValue(1) );


          // T of this node
          AutoDScalar  T = x[fvm_nodes[i]->local_offset()+1]; T.setADValue(0,1.0);

          AutoDScalar  T_0 = x[fvm_nodes[0]->local_offset()+3]; T_0.setADValue(1,1.0);

          // the T of this node is equal to corresponding T of semiconductor node
          AutoDScalar  f_T = T - T_0;

          // set Jacobian of governing equation f_T
          jac->add( fvm_nodes[i]->global_offset()+1,  fvm_nodes[i]->global_offset()+1,  f_T.getADValue(0) );
          jac->add( fvm_nodes[i]->global_offset()+1,  fvm_nodes[0]->global_offset()+3,  f_T.getADValue(1) );
 ...
 * update electrode IV
 */
void PolyGateContactBC::DDM2_Update_Solution(PetscScalar *)
{
