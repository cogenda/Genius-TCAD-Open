 ...
 */
void HomoInterfaceBC::DDM2_Function_Preprocess(Vec f, std::vector<PetscInt> &src_row,
        ...
        switch( region->type() )
        {
            case SemiconductorRegion :
            {
              ...
              for(; nb_it != fvm_nodes[i]->neighbor_node_end(); ++nb_it)
              {
                ...
                cols.push_back((*nb_it).second->global_offset()+3);
              }
 ...
 */
void HomoInterfaceBC::DDM2_Jacobian_Preprocess(Mat *jac, std::vector<PetscInt> &src_row,
    std::vector<PetscInt> &dst_row, std::vector<PetscInt> &clear_row)
{
  // search for all the node with this boundary type
  BoundaryCondition::const_node_iterator node_it = nodes_begin();
