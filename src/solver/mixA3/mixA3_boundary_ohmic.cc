 ...
 */
void OhmicContactBC::MixA_EBM3_Function_Preprocess(PetscScalar *,Vec f, std::vector<PetscInt> &src_row,
      ...
      switch ( regions[i]->type() )
      {
          ...
          case SemiconductorRegion:
          {
            ...
            if( semi_region->get_advanced_model()->enable_Tl() && (node_on_boundary(*node_it) || has_associated_region(*node_it, VacuumRegion)) )
            {
              ...
              y.push_back( fT + Heat_Transfer*(T_external()-T)*S );
              iy.push_back(iT);
            }
 ...
 */
void OhmicContactBC::MixA_EBM3_Jacobian_Preprocess(PetscScalar * ,SparseMatrix<PetscScalar> *jac, std::vector<PetscInt> &src_row,  std::vector<PetscInt> &dst_row, std::vector<PetscInt> &clear_row)
{
 ...
 */
void OhmicContactBC::MixA_EBM3_Jacobian(PetscScalar * x, SparseMatrix<PetscScalar> *jac, InsertMode &add_value_flag)
{
 ...
 * update electrode IV
 */
void OhmicContactBC::MixA_EBM3_Update_Solution(PetscScalar *)
{
