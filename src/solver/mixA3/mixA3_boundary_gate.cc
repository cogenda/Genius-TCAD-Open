  ...
  // first, we zero all the rows corresponding to GateContact bc.
  {
    ...
    for(; node_it!=end_it; ++node_it )
    {
      ...
      for(unsigned int i=0 ; rnode_it!=end_rnode_it; ++i, ++rnode_it  )
      {
        ...
        case InsulatorRegion:
          {
            row_for_clear.push_back(fvm_nodes[i]->global_offset() + regions[i]->ebm_variable_offset(POTENTIAL));
            break;
          }
        case ConductorRegion:
          {
            ...
            if(regions[i]->get_advanced_model()->enable_Tl())
            {
              ...
              row_for_clear.push_back(fvm_nodes[i]->global_offset() + regions[i]->ebm_variable_offset(TEMPERATURE));
            }
      ...
      case InsulatorRegion:
        {
          ...
          // the electrode potential in current iteration
          AutoDScalar Ve = x[ckt->local_offset(spice_node_index)];                         Ve.setADValue(1, 1.0);
          ...
          PetscInt cols[2] = {fvm_nodes[i]->global_offset()+node_psi_offset, ckt->global_offset(spice_node_index)};
