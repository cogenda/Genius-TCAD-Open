 ...
 */
void SolderPadBC::MixA_EBM3_Fill_Value(Vec x, Vec L)
{
...
/*---------------------------------------------------------------------
 * do pre-process to function for Mixed EBM3 solver
 */
void SolderPadBC::MixA_EBM3_Function_Preprocess(PetscScalar *,Vec f, std::vector<PetscInt> &src_row,
    std::vector<PetscInt> &dst_row, std::vector<PetscInt> &clear_row)
{
  ...
  for(; node_it!=end_it; ++node_it )
  {
    ...
    for ( ; rnode_it!=end_rnode_it; ++rnode_it )
    {
      ...
      clear_row.push_back(row + node_psi_offset);
    }
  }
}
 ...
 */
void SolderPadBC::MixA_EBM3_Function(PetscScalar * x, Vec f, InsertMode &add_value_flag)
{
    ...
    for ( ; rnode_it!=end_rnode_it; ++rnode_it )
    {
      ...
      switch ( region->type() )
      {
          case MetalRegion :
          {
            ...
            VecSetValue(f, fvm_node->global_offset()+node_psi_offset, f_psi, ADD_VALUES);
            ...
            if( region->get_advanced_model()->enable_Tl() && (node_on_boundary(*node_it) || has_associated_region(*node_it, VacuumRegion)) )
            {
              ...
              VecSetValue(f, fvm_node->global_offset()+node_Tl_offset, f_q, ADD_VALUES);
            }
 ...
 */
void SolderPadBC::MixA_EBM3_Jacobian_Reserve(Mat *jac, InsertMode &add_value_flag)
{
 ...
 */
void SolderPadBC::MixA_EBM3_Jacobian_Preprocess(PetscScalar * ,Mat *jac, std::vector<PetscInt> &src_row,
    std::vector<PetscInt> &dst_row, std::vector<PetscInt> &clear_row)
{
  BoundaryCondition::const_node_iterator node_it = nodes_begin();
  BoundaryCondition::const_node_iterator end_it = nodes_end();
 ...
 * build function and its jacobian for Mixed EBM3 solver
 */
void SolderPadBC::MixA_EBM3_Jacobian(PetscScalar * x, Mat *jac, InsertMode &add_value_flag)
{
      ...
      switch ( region->type() )
      {
          case MetalRegion :
          {
            ...
            if( region->get_advanced_model()->enable_Tl() && (node_on_boundary(*node_it) || has_associated_region(*node_it, VacuumRegion)) )
            {
              ...
              MatSetValue(*jac, fvm_node->global_offset()+node_Tl_offset, fvm_node->global_offset()+node_Tl_offset, f_q.getADValue(0), ADD_VALUES);
            }
            ...
            for(; nb_it != fvm_node->neighbor_node_end(); ++nb_it)
            {
              ...
              MatSetValue(*jac, bc_global_offset, fvm_node->global_offset()+node_psi_offset, current.getADValue(0), ADD_VALUES);
              MatSetValue(*jac, bc_global_offset, nb_node->global_offset()+node_psi_offset, current.getADValue(1), ADD_VALUES);
            }
            break;
          }
          case InsulatorRegion :
          {
            ...
            //governing equation
            MatSetValue(*jac, fvm_node->global_offset()+node_psi_offset, fvm_node->global_offset()+node_psi_offset, ff.getADValue(0), ADD_VALUES);
            MatSetValue(*jac, fvm_node->global_offset()+node_psi_offset, bc_global_offset, ff.getADValue(1), ADD_VALUES);
            break;
          }
          default: genius_error();
