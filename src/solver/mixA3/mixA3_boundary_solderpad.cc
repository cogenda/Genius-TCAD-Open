      ...
      case MetalRegion :
        {
          ...
          for(; nb_it != fvm_node->neighbor_node_end(); ++nb_it)
          {
            ...
            PetscScalar T_nb = nb_node_data->T();
            
            if(region->get_advanced_model()->enable_Tl())
            {
              // T of neighbor node
              T_nb = x[nb_node->local_offset()+1];
            }
 ...
 */
void SolderPadBC::MixA_EBM3_Jacobian_Preprocess(PetscScalar * ,SparseMatrix<PetscScalar> *jac, std::vector<PetscInt> &src_row,
    std::vector<PetscInt> &dst_row, std::vector<PetscInt> &clear_row)
{
  BoundaryCondition::const_node_iterator node_it = nodes_begin();
  BoundaryCondition::const_node_iterator end_it = nodes_end();
 ...
 * build function and its jacobian for Mixed EBM3 solver
 */
void SolderPadBC::MixA_EBM3_Jacobian(PetscScalar * x, SparseMatrix<PetscScalar> *jac, InsertMode &add_value_flag)
{
      ...
      case MetalRegion :
        {
          ...
          {
            // T of this node
            T = x[fvm_node->local_offset()+1];  T.setADValue(1, 1.0);
            // add heat flux out of boundary to lattice temperature equatiuon
          ...
          for(; nb_it != fvm_node->neighbor_node_end(); ++nb_it)
          {
            ...
            AutoDScalar T_nb = nb_node_data->T();
            
            if(region->get_advanced_model()->enable_Tl())
            {
              // T of neighbor node
              T_nb = x[nb_node->local_offset()+1];  T.setADValue(3, 1.0);
            }
 ...
 * update electrode IV
 */
void SolderPadBC::MixA_EBM3_Update_Solution(PetscScalar *)
{
