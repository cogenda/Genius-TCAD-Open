...
#ifdef __Dirichlet_bc__
            const SemiconductorSimulationRegion * semi_region = dynamic_cast<const SemiconductorSimulationRegion *>(region);

            if(semi_region->get_advanced_model()->QNEnabled)
            {
              unsigned int qn_offset = semi_region->dg_variable_offset(EQC);
              clear_row.push_back(row+qn_offset);
            }

            if(semi_region->get_advanced_model()->QPEnabled)
            {
              unsigned int qp_offset = semi_region->dg_variable_offset(EQV);
              clear_row.push_back(row+qp_offset);
            }
#endif
 ...
 */
void OhmicContactBC::DG_Jacobian_Preprocess(PetscScalar *, SparseMatrix<PetscScalar> *jac, std::vector<PetscInt> &src_row,
                                              std::vector<PetscInt> &dst_row, std::vector<PetscInt> &clear_row)
{
  ...
  for(node_it = nodes_begin(); node_it!=end_it; ++node_it )
  {
    ...
    for(; rnode_it!=end_rnode_it; ++rnode_it  )
    {
      ...
      if ( region->type() == SemiconductorRegion)
      {
        ...
        jac->get_row(  row[1],  n_variables,  &row[0],  &A1[0]);
        jac->get_row(  row[2],  n_variables,  &row[0],  &A2[0]);
        ...
        for(; nb_it != nb_it_end; ++nb_it)
        {
          ...
          jac->get_row(  row[1],  n_variables,  &col[0],  &A1[0]);
          jac->get_row(  row[2],  n_variables,  &col[0],  &A2[0]);
 ...
 * build function and its jacobian for Density Gradient solver
 */
void OhmicContactBC::DG_Jacobian(PetscScalar * x, SparseMatrix<PetscScalar> *jac, InsertMode &add_value_flag)
{
    ...
    for(unsigned int i=0 ; rnode_it!=end_rnode_it; ++i, ++rnode_it  )
    {
      ...
      switch ( regions[i]->type() )
      {
          ...
          case SemiconductorRegion:
          {
            ...
            {
              unsigned int qn_offset = semi_region->dg_variable_offset(EQC);
              jac->add( fvm_nodes[i]->global_offset()+qn_offset,  fvm_nodes[i]->global_offset()+qn_offset,  1.0 );
            }

            if(semi_region->get_advanced_model()->QPEnabled)
            {
              unsigned int qp_offset = semi_region->dg_variable_offset(EQV);
              jac->add( fvm_nodes[i]->global_offset()+qp_offset,  fvm_nodes[i]->global_offset()+qp_offset,  1.0 );
            }
#endif
            // displacement current
            if(SolverSpecify::TimeDependent == true)
