...
#ifdef HAVE_VTK
class MonitorHook::XMLUnstructuredGridWriter : public vtkXMLUnstructuredGridWriter
{
protected:
 ...
 */
MonitorHook::MonitorHook ( SolverBase & solver, const std::string & name, void * )
 ...
 */
void MonitorHook::post_iteration(void * f, void * x, void * dx, void * w, bool & , bool &)
{
  ...
  {
    std::cout<<"MonitorHook dump " << vtk_filename.str()<<std::endl;
  ...
  nonlinear_solver.dump_jacobian_matrix_petsc(matrix_filename.str());
  ...
  nonlinear_solver.dump_function_vector_petsc(vector_filename.str());
...
void MonitorHook::meshinfo_to_vtk()
{
...
std::string MonitorHook::export_extra_info()
{
  std::stringstream os;
  const SimulationSystem & system = _solver.get_system();
...
void MonitorHook::solution_to_vtk_ddm(void * _f, void * _x, void * _dx, void * _w)
{
  ...
  std::vector<float> fpsi, fn, fp;    // residual at this iteration
  std::vector<float> psi, n, p;    // solution at this iteration
  std::vector<float> dpsi, dn, dp; // newton update
  std::vector<float> psi_, n_, p_; // next solution

  Na.reserve(mesh.n_nodes());
  Nd.reserve(mesh.n_nodes());
  net_doping.reserve(mesh.n_nodes());
  fpsi.reserve(mesh.n_nodes());
  fn.reserve(mesh.n_nodes());
  fp.reserve(mesh.n_nodes());
  psi.reserve(mesh.n_nodes());
  n.reserve(mesh.n_nodes());
  p.reserve(mesh.n_nodes());
  dpsi.reserve(mesh.n_nodes());
  dn.reserve(mesh.n_nodes());
  dp.reserve(mesh.n_nodes());
  psi_.reserve(mesh.n_nodes());
  n_.reserve(mesh.n_nodes());
  p_.reserve(mesh.n_nodes());
...
void MonitorHook::solution_to_vtk_poisson(void * _f, void * _x, void * _dx, void * _w)
{
...
void MonitorHook::write_node_scaler_solution(const std::vector<unsigned int> & order, std::vector<float> &sol, const std::string & sol_name)
{
...
void MonitorHook::write_cell_scaler_solution(const std::vector<unsigned int> & order, std::vector<float> &sol, const std::string & sol_name)
{
  // this should run on parallel for all the processor
  Parallel::gather(0, sol);
...
extern "C"
{
  ...
  {
    return new MonitorHook ( solver, name, fun_data );
  }
