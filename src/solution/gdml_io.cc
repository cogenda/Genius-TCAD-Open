...
void GDMLIO::write_gdml_surface (const std::string& filename)
{
  ...
  if(Genius::processor_id() == 0)
  {
    ...
    << "xsi:noNamespaceSchemaLocation=\"schema/gdml.xsd\">" << std::endl << std::endl;
    ...
    for(unsigned int n=0; n<sl.size(); n++)
    {
      ...
      AutoPtr<Elem> boundary_side = elem->build_side(sl[n], false);
  ...
  for( unsigned int r=0; r<system.n_regions(); r++)
    ...
    for( unsigned int r=0; r<system.n_regions(); r++)
    {
      ...
      _out << "  <volume name=\"Volumn_" << region->name() << "\">" << std::endl;
    ...
    for( unsigned int r=0; r<system.n_regions(); r++)
    {
      ...
      _out << "       <volumeref ref=\"Volumn_" << region->name() << "\"/>" << std::endl;
      _out << "    </physvol>" << std::endl;
    }
    _out << "  </volume>" << std::endl;
...
void GDMLIO::write_gdml_body (const std::string& filename)
{
  ...
  for (; nd != nd_end; ++nd)
  {
    ...
    for( unsigned int r=0; r<system.n_regions(); r++)
    {
      ...
      Na[(*nd)->id()]  = node_data->Total_Na()/concentration_scale;
      Nd[(*nd)->id()]  = node_data->Total_Nd()/concentration_scale;
    }
  }
  ...
  if(Genius::processor_id() == 0)
  {
    ...
    {
      const Node * node = (*nd);
      _out << "  <position name=\"v" << node->id() << "\" unit=\"micron\" "
      << "x=\"" << node->x()/um <<"\" "
      << "y=\"" << node->y()/um <<"\" "
      << "z=\"" << node->z()/um <<"\"/>"
      <<std::endl;
    ...
    for (; elem_it != elem_end; ++elem_it)
    {
      ...
      _out << "  <material name=\"M_" << elem->id() <<"\" formula=\"" << region->material() <<"\">" <<std::endl;
      _out << "  <D value=\"" << density <<"\" />" <<std::endl;
      ...
      if(region->type()==SemiconductorRegion)
      {
        ...
        double d = density/molecule_value*6.2e23;
    ...
    {
      const Elem *elem  = (*elem_it);
      _out << "  <tessellated name=\"s" << elem->id() << "\">" << std::endl;

      for(unsigned int n=0; n<elem->n_sides(); n++)
      {
        AutoPtr<Elem> boundary_side = elem->build_side(n, false);
    ...
    {
      const Elem *elem  = (*elem_it);
      _out << "  <volume name=\"Volumn_" << elem->id() << "\">" << std::endl;
      _out << "  <materialref ref=\"M_" << elem->id() <<"\"/>" << std::endl;
      _out << "  <solidref ref=\"s" << elem->id() <<"\"/>" << std::endl;
      _out << "  </volume>" << std::endl;
    }
    ...
    for ( elem_it = mesh.active_elements_begin(); elem_it != elem_end; ++elem_it)
    {
      const Elem *elem  = (*elem_it);
      _out << "    <physvol>" << std::endl;
      _out << "       <volumeref ref=\"Volumn_" << elem->id() << "\"/>" << std::endl;
      _out << "    </physvol>" << std::endl;
    }
    _out << "  </volume>" << std::endl;
    ...
    _out << "<!-- minimal volumn size is " << minimal_size/um << " um -->" << std::endl;
