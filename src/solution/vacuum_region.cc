...
VacuumSimulationRegion::VacuumSimulationRegion(const std::string &name, const std::string &material, SimulationSystem & system)
:SimulationRegion(name, material, system) , mt( new Material::MaterialVacuum(material, name) )
{}
...
void VacuumSimulationRegion::insert_cell (const Elem * e)
{
  ...
  _region_cell_data.push_back(new FVM_Vacuum_CellData);
}
...
void VacuumSimulationRegion::init(PetscScalar T_external)
{
  ...
  for ( ; it!=_region_node.end(); ++it)
  {
    ...
    FVM_Vacuum_NodeData * node_data = dynamic_cast<FVM_Vacuum_NodeData *> ((*it).second->node_data());
