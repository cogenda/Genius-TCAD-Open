...
ConductorSimulationRegion::ConductorSimulationRegion(const std::string &name, const std::string &material, SimulationSystem & system)
:SimulationRegion(name, material, system) , mt( new Material::MaterialConductor(material, name) )
{

}

void ConductorSimulationRegion::insert_cell (const Elem * e)
{
  // not a local element
  ...
  _region_cell_data.push_back(new FVM_Conductor_CellData);
}


void ConductorSimulationRegion::init(PetscScalar T_external)
{
  //init FVM_NodeData
  ...
  for ( ; it!=_region_node.end(); ++it)
  {
    ...
    FVM_Conductor_NodeData * node_data = dynamic_cast<FVM_Conductor_NodeData *> ((*it).second->node_data());
...
void ConductorSimulationRegion::reinit_after_import()
{
