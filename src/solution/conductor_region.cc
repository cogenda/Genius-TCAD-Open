...
//  $Id: conductor_region.cc,v 1.6 2008/07/09 05:58:16 gdiso Exp $
...
void ElectrodeSimulationRegion::insert_cell (const Elem * e)
{
  // not a local element
...
void ElectrodeSimulationRegion::insert_fvm_node(FVM_Node * fn)
{
  // node (or ghost node) belongs to this processor
  // we should build FVM_NodeData structure for it

  if  ( fn->root_node()->on_local() )
    fn->hold_node_data( new FVM_Conductor_NodeData(&_node_data_storage, _region_point_variables) );

  _region_node[fn->root_node()->id()] = fn;
}


void ElectrodeSimulationRegion::set_region_variables()
{
  // preset named variales
  _node_data_storage.allocate_scalar_variable( std::vector<bool>(FVM_Conductor_NodeData::n_scalar(),false));
  _node_data_storage.allocate_complex_variable( std::vector<bool>(FVM_Conductor_NodeData::n_complex(),false));
  _node_data_storage.allocate_vector_variable( std::vector<bool>(FVM_Conductor_NodeData::n_vector(),false));
  _node_data_storage.allocate_tensor_variable( std::vector<bool>(FVM_Conductor_NodeData::n_tensor(),false));

  _cell_data_storage.allocate_scalar_variable( std::vector<bool>(FVM_Conductor_CellData::n_scalar(),false));
  _cell_data_storage.allocate_complex_variable( std::vector<bool>(FVM_Conductor_CellData::n_complex(),false));
  _cell_data_storage.allocate_vector_variable( std::vector<bool>(FVM_Conductor_CellData::n_vector(),false));
  ...
  _region_point_variables["potential.ac"  ] = SimulationVariable("potential.ac", COMPLEX, POINT_CENTER, "V", FVM_Conductor_NodeData::_psi_ac_, false);
  _region_point_variables["temperature.ac"] = SimulationVariable("temperature.ac", COMPLEX, POINT_CENTER, "K", FVM_Conductor_NodeData::_T_ac_, false);
  _region_point_variables["optical_efield"] = SimulationVariable("optical_efield", COMPLEX, POINT_CENTER, "V/cm", FVM_Conductor_NodeData::_OpE_complex_, false);
  _region_point_variables["optical_hfield"] = SimulationVariable("optical_hfield", COMPLEX, POINT_CENTER, "A/cm", FVM_Conductor_NodeData::_OpH_complex_, false);

  // define _region_cell_variables
  _region_cell_variables["efield"        ] = SimulationVariable("efield", VECTOR, CELL_CENTER, "V/cm", FVM_Conductor_CellData::_E_, true);
  _region_cell_variables["elec_current"  ] = SimulationVariable("elec_current", VECTOR, CELL_CENTER, "A/cm", FVM_Conductor_CellData::_Jn_, true);
  _region_cell_variables["hole_current"  ] = SimulationVariable("hole_current", VECTOR, CELL_CENTER, "A/cm", FVM_Conductor_CellData::_Jp_, true);
...
void ElectrodeSimulationRegion::init(PetscScalar T_external)
{
  //init FVM_NodeData
...
void ElectrodeSimulationRegion::reinit_after_import()
{
