dnl -------------------------------------------------------------
dnl $Id: aclocal.m4,v 1.9 2008/06/13 12:37:16 gdiso Exp $
dnl -------------------------------------------------------------
dnl
      ...
      if test "x$is_mips_pro" != "x" ; then
        ...
        if test "x$is_intel_ecc" != "x" ; then
          ...
          if test "x$is_intel_icc" != "x" ; then
			...
			AC_MSG_RESULT( make.defs directly to specify)
  ...
  if (test "$GXX" = yes -a "x$REAL_GXX" != "x" ) ; then
    ...
    case $host_os in
      *darwin*)
      ;;
      *)
        dnl export dynamic link symbol
	...
	dnl The -g flag is all OProfile needs to produce annotations
        ...
        case "$GXX_VERSION" in
          ...
          intel_icc_v10.0| intel_icc_v10.1)
              ...
              CFLAGS_DVL="$CFLAGS_DBG"
	      dnl Intel compilers >10.0 use -p for profiling
	      PROFILING_FLAGS="-p"
	      ;;
...
[
  ...
  if test "x$PETSC_ARCH" = x ; then
    ...
    if (test -r $PETSC_DIR/include/petsc.h) ; then
      ...
      if test "$petscrelease" = 1 ; then
        AC_MSG_RESULT(<<< Configuring library with PETSc version $petscversion support >>>)
        case $petscmajor in
          2*)
             PETSCBMAKEVAR='bmake/common/variables'
             ;;
          3*)
             PETSCBMAKEVAR='conf/variables'
             ;;
        esac
      else
        AC_MSG_RESULT(<<< Configuring library with PETSc-devel >>>)
        PETSCBMAKEVAR='conf/variables'
        AC_DEFINE(HAVE_PETSC_DEV, 1,
            [Flag indicating whether we are using the devel version of PETSc])
      fi
      AC_SUBST(PETSCBMAKEVAR)
...
dnl ANN
dnl -------------------------------------------------------------
AC_DEFUN(CONFIGURE_ANN,
[
  AC_CHECK_FILE(./src/contrib/ann/ANN.cpp,
	        [
	          ANN_INCLUDE_PATH=$PWD/src/contrib/ann
                  ANN_INCLUDE=-I$ANN_INCLUDE_PATH
                  ANN_LIB="\$(EXTERNAL_LIBDIR)/libann\$(libext)"
		  AC_SUBST(ANN_INCLUDE)
                  AC_SUBST(ANN_LIB)
                  AC_DEFINE(HAVE_ANN, 1,
	                    [Flag indicating whether or not ANN is available])
                  AC_MSG_RESULT(<<< Configuring library with ANN support >>>)
	          enableann=yes
                ],
                [enableann=no])
])
...
[
  AC_CHECK_FILE(./src/contrib/metis/metis.h,
	        [
	          METIS_INCLUDE_PATH=$PWD/src/contrib/metis
...
[
  ...
  VTK_INC="/usr/local/include/vtk-5.4"
  VTK_LIB="/usr/local/lib/vtk-5.4"
  ...
  if (test $enablevtk = yes); then
     ...
     AC_CHECK_FILE([$VTK_INC/vtkConfigure.h], [vtkIncFound="yes"], [vtkIncFound="no"])
     ...
     case $host_os in
       *darwin*)
         AC_CHECK_FILE($VTK_LIB/libvtkIO.a, [vtkLibFound=yes], [vtkLibFound=no]);;
       *linux* )
         AC_CHECK_FILE($VTK_LIB/libvtkIO.a, [vtkLibFound=yes], [vtkLibFound=no]);; 
       *cygwin* )
         AC_CHECK_FILE($VTK_LIB/vtkIO.lib, [vtkLibFound=yes], [vtkLibFound=no]);; 
     esac
 
     if (test $vtkLibFound = no); then
       AC_MSG_RESULT(VTK library files not found!)
       enablevtk=no;
     fi

     dnl If both the header file and the required libs were found, continue.
     if (test $vtkIncFound = yes -a $vtkLibFound = yes); then
       ...
       case $host_os in
       *darwin*)
          VTK_LIBRARY="-L$VTK_LIB -lvtkRendering -lvtkGraphics -lvtkImaging -lvtkIO -lvtkFiltering -lvtkCommon -lvtksys"
          VTK_LIBRARY="$VTK_LIBRARY -lvtkDICOMParser -lvtkpng -lvtktiff -lvtkzlib -lvtkjpeg -lvtkexpat -lvtkftgl -lvtkfreetype"
          VTK_LIBRARY="$VTK_LIBRARY -framework Carbon"
          ;;
       *linux* )
          VTK_LIBRARY="-L$VTK_LIB -lvtkRendering -lvtkGraphics -lvtkImaging -lvtkIO -lvtkFiltering -lvtkCommon -lvtksys"
          VTK_LIBRARY="$VTK_LIBRARY -lvtkDICOMParser -lvtkpng -lvtktiff -lvtkzlib -lvtkjpeg -lvtkexpat -lvtkftgl -lvtkfreetype"
          ;;
       *cygwin* )
          VTK_LIBRARY="-L$VTK_LIB vtkRendering.lib vtkGraphics.lib vtkImaging.lib vtkIO.lib vtkFiltering.lib vtkCommon.lib vtksys.lib"
          VTK_LIBRARY="$VTK_LIBRARY vtkDICOMParser.lib vtkpng.lib vtktiff.lib vtkzlib.lib vtkjpeg.lib vtkexpat.lib vtkftgl.lib vtkfreetype.lib"
          ;;
       esac
