// $Id: dense_vector.h 2789 2008-04-13 02:24:40Z roystgnr $
...
{
  assert (this->size());
  Real my_min = genius_real((*this)(0));

  for (unsigned int i=1; i!=this->size(); i++)
    {
      Real current = genius_real((*this)(i));
...
{
  assert (this->size());
  Real my_max = genius_real((*this)(0));
...
Real DenseVector<T>::linfty_norm () const
{
  if (!this->size())
    return 0.;
  Real my_norm = genius_norm((*this)(0));

  for (unsigned int i=1; i!=this->size(); i++)
    {
      Real current = genius_norm((*this)(i));
