// $Id: mesh_refinement.h,v 1.2 2008/03/20 13:33:40 gdiso Exp $
...
public:
  ...
  void flag_elements_by_error_fraction (const ErrorVector& error_per_cell,
					...
					const unsigned int max_level = invalid_uint);
  ...
  /**
   * Flags elements for coarsening and refinement based on
   * the computed error passed in \p error_per_cell.  
   * cell error large than \p refine_threshold will be refined
   * and cell error less than \p coarsen_threshold will be coarsened
   */
  void flag_elements_by_error_threshold (const ErrorVector& error_per_cell,
   ...
   * reach.
   *
   * \p max_h_level is unlimited (invalid_uint) by default
   */
...
private:
  ...
  /**
   * for element pairs lie on interface, we'd better make them compatible   
   */
  bool make_interface_compatible ();
