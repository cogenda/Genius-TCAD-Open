...
/**
 * Solve the Density Gradient Model equations!
 */
class DGSolver : public DDMSolverBase
{
public:
  DGSolver(SimulationSystem & system): DDMSolverBase(system)
  ...
  {return SolverSpecify::DENSITY_GRADIENT;}
  ...
  virtual unsigned int node_dofs(const SimulationRegion * region) const
  {
    switch(region->type())
    {
      ...
      {
        unsigned int dof = 3;
        if(region->advanced_model().QNEnabled) dof++;
        if(region->advanced_model().QPEnabled) dof++;
        return dof; //semiconductor node has max 5 dof
      }
  ...
  virtual unsigned int bc_node_dofs(const BoundaryCondition * bc) const
  {
    switch (bc->bc_type())
    {
      case OhmicContact      : return 5; // ohmic electrode current
      case SchottkyContact   : return 5; // displacement current
