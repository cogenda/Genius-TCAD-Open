...
/**
 * Mixed type simulation with Level 2 Drift-Diffusion Model equations
 */
class Mix2Solver : public MixSolverBase
{
public:
  Mix2Solver(SimulationSystem & system): MixSolverBase(system)
  ...
  virtual unsigned int node_dofs(const SimulationRegion * region) const
  {
    switch(region->type())
    {
      case SemiconductorRegion : return 4; //semiconductor node has 3 dof
      case InsulatorRegion     : return 2; //insulator node has 1 dof
      case ConductorRegion     : return 2; //conductor node has 1 dof
      default : return 0;
    }
  }
