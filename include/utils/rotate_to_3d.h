...
#ifndef __rotate_to_3d_h__
#define __rotate_to_3d_h__
...
/**
 * give a 2D mesh (tri/quad), rotate it to 3D prismatic mesh.
...
public:
  ...
  RotateTo3D(SimulationSystem & system, const Parser::Card & c)
...
private:
  ...
  /**
   *  set of on axis point
   */ 
  std::set<unsigned int> on_axis_node;
  
  /**
   *  set of off axis point
   */ 
  std::set<unsigned int> off_axis_node;
  ...
  /**
   * node chain in theta direction 
   */ 
  std::vector< std::vector<Node *> > _node_chain;

  
  /**
   * create rotated elems from tri elem 
   */  
  std::vector<Elem *> rotate_elem(unsigned int A, unsigned int B, unsigned int C); 
  
  /**
   * create rotated elems from quad elem 
   */  
  std::vector<Elem *> rotate_elem(unsigned int A, unsigned int B, unsigned int C, unsigned int D); 
