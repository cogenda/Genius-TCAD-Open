// $Id: tree_node.h,v 1.3 2008/05/22 14:14:19 gdiso Exp $
...
public:
  ...
  /**
   * Inserts \p Elem \p el into the TreeNode.
   * b1 and b2 are two optional bound box point
   */
  void insert (const Elem* nd, const Point * b1, const Point *b2);
   ...
   * all the elements in this tree node will in the elem_bounding_box
   */
  void build_elem_bounding_box();

  /**
   * @return the elem_bounding_box
   */
  const std::pair<Point, Point>& get_elem_bounding_box() const;
  ...
  /**
   * @return true if the ray hit the bounding_box
   */
  bool hit_boundbox(const Point & p, const Point & dir, std::pair<double, double>&) const;
...
private:
   ...
   * NOTE some elements may only intersect with bounding_box, not in bounding_box
   */
  std::pair<Point, Point> bounding_box;

  /**
   * The Cartesian bounding box for the elem.
   * NOTE all the elements will in bounding_box
   */
  std::pair<Point, Point> elem_bounding_box;
